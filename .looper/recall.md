...
On 2026-08-29 the three C# tests went red on CI six times across 22.18.0, 26.x and 24.x — "the C# reader answers at all" said 'unavailable', and "every C# case agrees with the rule it was written from" reported cases disagreeing. No rule was wrong: the reader had not answered, and its detail was "Command failed: …/looper-csharp …" with nothing after it, because src/law/csharp/drive.ts ran the binary with stderr set to ignore. PR #178 read that silence as a transient fault. It was not: since #177 two test files build the C# engine on a fresh checkout, node --test runs them in parallel processes, and two dotnet builds in one directory rewrite bin/ under each other — a reader run meanwhile exits 154 "The application to execute does not exist" or 147 "Failed to map file … runtimeconfig.json". Reproduced three rounds out of three; the fix serialises the build with a lock and keeps the exit status and the last words in the detail.

For the next session: when C# cases "disagree with their rule" on CI and nothing about the rules changed, read the "the C# reader answers at all" result in tests/csharp-cases.test.ts first — it separates a reader that did not start from rules that are wrong, and its detail now names the exit status and what the reader said. A silence there is a bug in the driver, never a re-run.
